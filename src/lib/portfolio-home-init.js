...
function initNavSpy() {
  ...
  homeDisposers.push(() => window.removeEventListener('scroll', onScroll));
...
function initStatsReveal() {
  ...
  homeDisposers.push(() => observer.disconnect());
}
...
const MODEL_URL = '/assets/model/scene-opt.glb';
...
function loadColoredSkillIconUrl(skill, dark = document.documentElement.dataset.theme === 'dark') {
  ...
  return new Promise(resolve => {
    ...
    img.onerror = () => {
      ...
      fallback.onerror = () => resolve(getSkillIconAssetUrl(skill));
...
const SKILL_DEFS = [
  ...
  { slug: 'csharp', color: '#512BD4', label: 'C#', group: 'vrxr', desc: 'Primary scripting language for Unity gameplay, XR interactions, and tooling.' },
  {
    ...
    label: 'WebXR',
    group: 'vrxr',
    desc: 'Browser-based VR/AR APIs for immersive experiences on the open web.',
...
function initPanelModals() {
  ...
  window.addEventListener('rv-theme-change', onThemeChange);
  homeDisposers.push(() => {
    window.removeEventListener('keydown', onKeydown);
    window.removeEventListener('rv-theme-change', onThemeChange);
  });
}
...
export function initHome() {
  // Clear any listeners from a previous mount (SPA re-navigation / StrictMode).
  homeDisposers.forEach(fn => fn());
  homeDisposers = [];
  ...
  const onResize = () => syncFeaturedProjectsScroll(document.getElementById('projectsList'));
  window.addEventListener('resize', onResize);
  homeDisposers.push(() => window.removeEventListener('resize', onResize));
  ...
  window.addEventListener('rv-theme-change', onThemeChange);
  homeDisposers.push(() => window.removeEventListener('rv-theme-change', onThemeChange));
...
export function initThreeScene(canvas) {
  ...
  return new RohanPrefabPortfolioScene(canvas);
}
